...
class Unions:
    def __init__(self):
        ...
        self.sds2 = {
            "cdmk_valid": 0,  # Исправность ЦДМК
            "cdmk_failure": 0,  # Отказ ЦДМК
            "cdmk_link": 0,  # Наличие связи с ЦДМК по CAN
            "bit4": 0,
            "ufos1_valid": 0,  # Исправность УФОС-01 №1
            "ufos1_failure": 0,  # Отказ УФОС-01 №1
            "fos1_link": 0,  # Наличие связи с УФОС-01 по CAN №1
            "ufos1_work": 0,  # УФОС-01 №1 в режиме "Работа"
            "ufos2_valid": 0,  # Исправность УФОС-01 №2
            "ufos2_failure": 0,  # Отказ УФОС-01 №2
            "ufos2_link": 0,  # Наличие связи с УФОС-01 по CAN №2
            "ufos2_work": 0,  # УФОС-01 №2 в режиме "Работа"
            "bit13": 0,
            "bit14": 0,
            "bit15": 0,
            "bit16": 0,
        ...
        self.sds1_data = struct.pack(
            "!H", int("".join(str(self.sds1[key]) for key in self.sds1), 2)
        )
        self.sds2_data = struct.pack(
            "!H", int("".join(str(self.sds2[key]) for key in self.sds2), 2)
        )
        self.sds3_data = struct.pack(
            "!H", int("".join(str(self.sds3[key]) for key in self.sds3), 2)
        )
        self.sds2X_data = struct.pack(
            "!H", int("".join(str(self.sds2X[key]) for key in self.sds2X), 2)
        )
        self.sds3X_data = struct.pack(
            "!H", int("".join(str(self.sds3X[key]) for key in self.sds3X), 2)
        )
